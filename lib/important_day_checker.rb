...
class ImportantDayChecker
  ...
  def check_days important_days, day, chat_id
    important_days.each do |name, birthday_date|
      today = Date.today
      if today.month == birthday_date.month and today.day == birthday_date.day
        text = "Happy #{day} @#{name}!"
        MessageSender.new(bot: bot, chat: nil, text:text).send_wishes_message chat_id
        MessageSender.new(bot: bot, chat: nil, text: text).send_wishes_message config.group_id
        MessageSender.new(bot: bot, chat: nil, text: text).send_wishes_message config.channel_id
      end
    end
  end
